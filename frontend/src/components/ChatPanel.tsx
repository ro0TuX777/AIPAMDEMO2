...
export function ChatPanel(props: ChatPanelInputProps) {
  ...
  const emit = useCallback((messages: ChatMessage[], id = conversation.id) => {
    messagesRef.current = messages;
    onConversationChanged({ ...conversation, ...(id ? { id } : {}), updated_at: new Date().toISOString(), messages });
  }, [conversation, onConversationChanged]);

  const updateAssistant = useCallback((requestId: string, update: (message: ChatMessage) => ChatMessage) => {
    emit(messagesRef.current.map(message => message.request_id === requestId && message.role === "assistant" ? update(message) : message));
  }, [emit]);

  const complete = useCallback((requestId: string, terminal: ChatStreamTerminalMetadata, content: string) => {
    ...
    emit(messagesRef.current.map(message => message.request_id === requestId && message.role === "assistant"
  ...
  const handleSend = useCallback(async (messageText?: string) => {
    ...
    const userMessage: ChatMessage = { id: newId(), sequence: nextSequence, role: "user", content: text, citations: [], metadata: null, request_id: requestId, timestamp };
    const assistantMessage: ChatMessage = { id: newId(), sequence: nextSequence + 1, role: "assistant", content: "", citations: [], metadata: { status: "pending" }, request_id: requestId, timestamp };
    emit([...conversation.messages, userMessage, assistantMessage]);
    setInput("");
    setPanelError(null);
    setIsLoading(true);
    const body = { message: text, conversation_id: conversation.id, context_hint: contextHint, mode, request_id: requestId };
    const applyJsonResponse = (response: Awaited<ReturnType<typeof api.chatWithJob>>) => complete(requestId, {
    ...
    try {
      if (isDemoMode()) {
        applyJsonResponse(await api.chatWithJob(jobId, body));
        return;
      }
      let content = "";
      const terminal = await api.streamWithJob(jobId, body, event => {
        ...
        if (event.type !== "error") updateAssistant(requestId, message => ({ ...message, content }));
      });
      complete(requestId, terminal, content);
      if (terminal.status === "error") setPanelError(mode === "mnemos" ? "MNEMOS could not complete this response." : "Chat could not complete this response.");
    } catch (error) {
      if (mode === "baseline" && error instanceof ChatStreamError && !error.responseReceived) {
        try {
          ...
          updateAssistant(requestId, assistant => ({ ...assistant, content: message, metadata: { status: "error" } }));
  ...
  }, [complete, contextHint, conversation.id, conversation.messages, emit, input, isLoading, jobId, mode, updateAssistant]);
  ...
  return <div className="flex flex-col h-full bg-slate-900 rounded-lg border border-slate-700">
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      {conversation.messages.map(message => <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[80%] rounded-lg px-4 py-2 ${message.role === "user" ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-100"}`}>
        ...
        {message.role === "user" && mode === "baseline" && controlled && <button type="button" onClick={() => onCopyToMnemos(message.id, message.content)} className="mt-2 text-xs underline underline-offset-2 hover:text-slate-200">Copy to MNEMOS</button>}
      ...
      {mode === "mnemos" && panelError && <button type="button" onClick={() => { setPanelError(null); onRetry(); }} className="text-sm text-amber-300 underline underline-offset-2">Retry MNEMOS</button>}
