...
import type { ChatComparisonBranch, ChatComparisonGroup } from "../api";
import type { ChatConversation, HistoricalFindingCitation } from "./chatTypes";
...
export function MnemosChatDrawer({
  ...
  const content = (
    <aside role={mobile ? "dialog" : "complementary"} aria-modal={mobile || undefined} aria-label="MNEMOS comparison" className="fixed inset-y-0 right-0 z-40 flex min-h-0 w-full max-w-lg flex-col overflow-hidden border-l border-slate-700 bg-slate-950 shadow-2xl md:static md:h-full md:w-[28rem] md:rounded-lg md:border">
      ...
      <div className="shrink-0 border-b border-slate-800 px-4 py-3">
        ...
        <select id="mnemos-branch" value={activeBranch.id} disabled={busy} onChange={event => onBranchChange(event.target.value)} className="mt-1 w-full rounded border border-slate-700 bg-slate-900 px-2 py-1.5 text-sm text-slate-100 disabled:opacity-50">
