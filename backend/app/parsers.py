        ...
        try:
            ...
            records.append(
                FlowRecord(
                    ...
                    app_proto=str(rec.get("service", "UNKNOWN")).upper(),
                    start_time=start,
                    end_time=end,
                    ...
                    extra={k: str(v) for k, v in rec.items() if k not in {
...
def parse_suricata_eve(logs: Iterable[dict]) -> List[AlertRecord]:
    ...
    for rec in logs:
        ...
        try:
            ...
            alerts.append(
                AlertRecord(
                    id=str(rec.get("flow_id") or rec.get("alert", {}).get("signature_id") or rec["timestamp"]),
                    timestamp=ts,
                    ...
                    extra={k: v for k, v in rec.items() if k not in {"timestamp", "src_ip", "src_port", "dest_ip", "dest_port", "dst_ip", "dst_port", "flow_id", "host", "event_type", "alert"}},
                )
            )
        except Exception:
            continue
