-- Frozen V1/V2 create-all schema reconstructed from 26170e2686504edce4d2f91a7ea345084382c3b7; profile partial_comparison.
-- Existing chat tables retain the precomparison shape; only new tables/indexes/trigger were created.
...
CREATE TABLE chat_comparison_branches (
	id VARCHAR NOT NULL,
	group_id VARCHAR NOT NULL,
	conversation_id VARCHAR NOT NULL,
	label TEXT NOT NULL,
	source_message_id VARCHAR,
	history_cutoff_sequence INTEGER NOT NULL,
	created_at VARCHAR NOT NULL,
	updated_at VARCHAR NOT NULL,
	PRIMARY KEY (id),
	FOREIGN KEY(group_id) REFERENCES chat_comparison_groups (id) ON DELETE CASCADE,
	FOREIGN KEY(conversation_id) REFERENCES chat_conversations (id) ON DELETE CASCADE,
	FOREIGN KEY(source_message_id) REFERENCES chat_messages (id) ON DELETE NO ACTION DEFERRABLE INITIALLY DEFERRED
)

;
CREATE INDEX idx_chat_comparison_branch_group ON chat_comparison_branches (group_id);
CREATE UNIQUE INDEX uq_chat_comparison_branch_conversation ON chat_comparison_branches (conversation_id);
CREATE UNIQUE INDEX uq_chat_comparison_snapshot_group ON chat_comparison_branches (group_id) WHERE source_message_id IS NULL;

CREATE TABLE chat_comparison_groups (
	id VARCHAR NOT NULL,
	job_id VARCHAR NOT NULL,
	root_conversation_id VARCHAR NOT NULL,
	title TEXT,
	...
	PRIMARY KEY (id),
	FOREIGN KEY(job_id) REFERENCES jobs (job_id) ON DELETE CASCADE,
	FOREIGN KEY(root_conversation_id) REFERENCES chat_conversations (id) ON DELETE CASCADE,
	FOREIGN KEY(active_branch_id) REFERENCES chat_comparison_branches (id) ON DELETE SET NULL
)

;
CREATE INDEX idx_chat_comparison_group_job ON chat_comparison_groups (job_id);
CREATE UNIQUE INDEX uq_chat_comparison_group_root ON chat_comparison_groups (root_conversation_id);
...
INSERT INTO chat_comparison_groups (id, job_id, root_conversation_id, created_at, updated_at) VALUES ('g1', 'j1', 'c1', '2026-01-01', '2026-01-01');
INSERT INTO chat_comparison_branches (id, group_id, conversation_id, label, history_cutoff_sequence, created_at, updated_at) VALUES ('b1', 'g1', 'c2', 'historical comparison', 1, '2026-01-01', '2026-01-01');
