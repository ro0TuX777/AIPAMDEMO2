...
def _finding_to_item(f: Finding) -> FindingItem:
    ...
    return FindingItem(
        ...
        evidence_status=getattr(f, "evidence_status", None) or "observed",
        corroboration_score=getattr(f, "corroboration_score", 0.0) or 0.0,
        corroborating_sources=f.corroborating_sources,
...
def _finding_to_detail(db: Session, finding: Finding) -> FindingDetailResponse:
    ...
    return FindingDetailResponse(
        ...
        evidence_status=getattr(finding, "evidence_status", None) or "observed",
        corroboration_score=getattr(finding, "corroboration_score", 0.0) or 0.0,
